...
**關鍵結果**
- 多空策略抗跌性明確：在市場回落區間，策略組回撤普遍小於基準。
- 風險調整後績效優於大盤：EW Sharpe 為 **0.847**，高於同期間基準 **0.779**。
- 在本研究設定下，OLS 篩選因子搭配 EW（等權）配置整體表現最佳。

**未來發展**
- 可依市況動態調整多空權重與檔數（如 `5/5`、`6/4`、`8/2`）。
- 可導入機器學習做因子篩選，提升不同市場環境下的適應性。
